...
namespace LW_Web.Controllers
{
    ...
    */
    ...
    public class WorkOrdersApiController : Controller
    {
            ...
            try
            {
                ...
                if (request.IncludeWOItems.GetValueOrDefault(false) && workOrders.Count > 0)
                {
                    foreach (var workOrder in workOrders)
                    {
                        ...
                        if (woNumber <= 0)
                        {
                            workOrder["WorkOrderItems"] = new List<Dictionary<string, object>>();
                            continue;
                        }

                        try
                        {
                            workOrder["WorkOrderItems"] = GetWorkOrderItems(woNumber, request.ItemCodes, request.FilterItemCategories);
                        }
        ...
        private List<Dictionary<string, object>> GetWorkOrderItems(int woNumber, List<string> itemCodes, List<string> filterItemCategories)
        {
            return _workOrdersData.GetWorkOrderItems(woNumber, itemCodes, filterItemCategories);
        }
